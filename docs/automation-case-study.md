...
### September 21, 2026 — A scheduled run kept nothing

- Followed the structured report through the scheduled path and found it stopped
  short. `SupervisorDispatcher` returned Markdown only, and `Scheduler.run_due`
  dispatched each due routine, recorded that it ran, and discarded the report.
  The `run_ledger` table has carried a `report_path` column since the scheduler
  was added; nothing had ever written to it.
- So a scheduled Milou could tell an operator that a routine had run and whether
  it failed, but not what it found. The archive only ever contained reports
  generated by hand or by the separate generation callable.
- Decided that `run_due` takes an optional report store, and that `run` takes
  `--store`. The report is persisted with its structure, and the stored path is
  written to the ledger so a run can be traced to its output. `DispatchResult`
  now carries the structured report alongside the Markdown.
- A storage failure is recorded as a run failure rather than passing silently,
  which matches the project's rule that failures stay visible: a run that
  produced a report nobody can find has not really succeeded.
- Lesson: the dead column was the tell. A schema that reserves a place for
  something the code never writes is usually a feature that was designed and
  then quietly dropped at the last step — the same shape of defect as the
  reports that computed a risk value and never used it.
